...
## FIXED VERSION - corrects dimensionality issues with a.gl matrix
...
if(mult.files){
	...
	for(locus in 1:nloci){
		...
		# Extract genotype likelihoods for this locus across all individuals
		locus_gl <- unlist(a.gl[locus, ])
		write.table(t(locus_gl), file=paste0(fname, ".mpgl"), append=T,
					col.names=F, row.names=F, eol=" ")
	}
}
...
if(mult.files){
	...
	# Process each cell of the genotype likelihood matrix
	for(i in 1:nloci){
		for(j in 1:nind){
			gl_values <- unlist(a.gl[i,j])
			temp <- sum(10^(-0.1*gl_values))
			a.mean.gl[i,j] <- sum((10^(-0.1*gl_values))*(0:max(ploidy)))/temp
		}
	}
}
...
## MODIFIED: Changed to generate K=1 through K=4 (instead of K=1 through K=3)
for(k in 1:4){
